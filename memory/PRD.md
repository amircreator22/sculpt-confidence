...
## SEO production fix — static-build compatible (June 2026)
- ROOT CAUSE of SEO not working in prod: production serves a PLAIN STATIC build (yarn build -> R2 -> Cloudflare edge), NOT craco `yarn start`. The earlier craco devServer.setupMiddlewares crawler middleware (seo-middleware.js) only runs in the dev server (preview), never in prod. Edge forwards only /api/* to backend; root /robots.txt, /sitemap.xml and bot page requests hit the static SPA layer. Backend /api/_seo/* routes are live but unreachable at root. (Confirmed by deployer agent; deploy pipeline itself was green.)
- FIX (static-build compatible): build-time prerender. frontend/package.json build = "craco build && node scripts/prerender-seo.js". prerender-seo.js reads build/index.html + scripts/seo-data.json and writes a static HTML file per route (build/index.html home, build/<route>/index.html others) with unique title/meta/canonical/OG/JSON-LD injected into <head>; keeps hashed JS/CSS bundles so humans still hydrate the SPA. Never fails the build (try/catch, exit 0).
- Static files: frontend/public/robots.txt + frontend/public/sitemap.xml (baked into build root by CRA).
- Data generation: backend/gen_seo_static.py regenerates scripts/seo-data.json + public/sitemap.xml + public/robots.txt from live /api/products via backend/seo.py (HOST hardcoded https://sculptivauk.com). Re-run this script + redeploy whenever the catalog changes.
- seo-middleware.js + craco.config.js hook kept for the preview/dev server (harmless in prod). Verified locally: yarn build logged "prerender-seo: baked SEO into 21 route(s)"; per-route titles + Product/CollectionPage JSON-LD + main.js bundle all present.
- RESIDUAL LIMITATION on static edge: unknown URLs still return 200 SPA (no per-request 404 possible without a server/edge worker). Prerender assumes the Cloudflare static edge serves nested /<route>/index.html for clean URLs — verify on live after deploy.

## 5 curated SEO collection pages (June 2026)
- Added 5 style/attribute-based collections reusing the existing /collections/:handle route (no route change): seamless-gym-leggings, scrunch-bum-leggings, high-waisted-gym-leggings, black-gym-leggings, squat-proof-leggings. Not separate Shopify collections — filtered from catalogue by product handle.
- Content source of truth mirrored in TWO places (keep in sync): frontend/src/data/curatedCollections.js (human page) and backend/seo.py CURATED (prerender/SEO). Each has title tag, meta desc, H1, body copy, 3 FAQs, breadcrumb trail, product handle list, hero image.
- Product mapping: seamless=[seamless-sculpt-leggings]; scrunch=[glute-sculpt-leggings,sculptflex-contour-leggings]; high-waisted/black/squat-proof=[glute-sculpt-leggings,seamless-sculpt-leggings,sculptflex-contour-leggings]. Intentional overlap.
- seo.py render_page: curated branch emits CollectionPage + ItemList + FAQPage + BreadcrumbList JSON-LD, and a rich baked body (intro + Featured styles + FAQs + Shop-by-style links). Added breadcrumb_ld() and generalized _faq_ld(pairs).
- Prerender upgrade: seo-data.json entries are now {head, body?}; prerender-seo.js injects head for all routes and injects body into <div id="root"> ONLY for the 5 curated routes (existing 21 routes unchanged, head-only). Body is replaced when React hydrates — humans get full SPA page.
